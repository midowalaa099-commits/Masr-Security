        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div x-data="{
                ...
                add() {
                    ...
                    this.items.push({ id: null, product_id: '', quantity: 1 });
                },
                remove(index) { this.items.splice(index, 1); },
                productName(id) { return this.products.find((p) => String(p.id) === String(id))?.name ?? ''; },
                ...
                init() { this.$watch('items', () => this.recalculate()); },
            }" class="mt-4">
                <template x-for="(item, index) in items" :key="index">
                    <div class="mb-3 grid grid-cols-1 gap-3 sm:grid-cols-[1.5fr_1fr_auto_auto]">
                        ...
                        <input type="number" min="1" x-model.number="item.quantity" :name="`items[${index}][quantity]`" x-on:input="recalculate()"
                            class="rounded-lg border-slate-300 px-3 py-2 text-sm">
                        <span class="self-center text-sm font-semibold text-slate-500" x-html="item.product_id ? (productPrice(item.product_id) * (item.quantity || 1)).toFixed(2) : ''"></span>
